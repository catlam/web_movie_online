...
const deleteFavoriteMovieByIdAction = ({ id, kind }) => async (dispatch, getState) => {
    try {
        dispatch({ type: userConstants.DELETE_FAVORITE_MOVIE_REQUEST});
        await userApi.deleteFavoriteMovieService({ id, kind }, tokenProtection(getState));
...
const likeMovieAction = ({ id, kind }) => async (dispatch, getState) => {
    try {
        dispatch({ type: userConstants.LIKE_MOVIE_REQUEST});
        const response = await userApi.likeMovieService({ id, kind }, 

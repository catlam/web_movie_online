...
export const getAllMoviesAction = ({
    ...
    try {
        ...
        const response = await moviesAPIs.getAllMoviesServices(
            category,
            time,
            language,
            rate,
...
export const getRandomMoviesAction = (size = 8) => async (dispatch) => {
    try {
        dispatch({ type: moviesConstants.MOVIES_RANDOM_REQUEST })
        const response = await moviesAPIs.getRandomMoviesService(size)
        dispatch({
